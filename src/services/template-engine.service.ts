...
class MemoryTemplateCache implements TemplateCache {
  ...
  private evictLeastRecentlyUsed(): void {
    let lruTemplateId: string | null = null;
  ...
  private calculateTemplateSize(template: CompiledTemplate): number {
...
export class HandlebarsTemplateEngine implements TemplateEngine {
  ...
  private storage: SupabaseTemplateStorage;
  private trackingService: TrackingUrlService;
  ...
  async renderTemplate(
    ...
    try {
      ...
      if (compiledTemplate.html) {
        ...
        renderedHtml = this.processHtmlTracking(
          renderedHtml,
          templateId,
          context
        );
      ...
      return {
        ...
        metadata: {
          ...
          variables_used: this.extractUsedVariables(
            compiledTemplate,
            enhancedContext
          ),
        },
  ...
  async compileTemplate(template: EmailTemplate): Promise<CompiledTemplate> {
    ...
    const templateHash = crypto.createHash("md5").update(content).digest("hex");
  ...
  async renderPreview(
    ...
    if (sampleData) {
      ...
      if (sampleData.email) sampleContext.contact.email = sampleData.email;
    }

    // Use temporary template ID for preview
    const tempTemplate: CompiledTemplate = {
      id: `preview-${uuidv4()}`,
      name: template.name,
      subject: template.subject,
      html: template.html_content || "",
  ...
  extractVariables(templateContent: string): TemplateVariable[] {
    ...
    while ((match = variablePattern.exec(templateContent)) !== null) {
      ...
      if (
        this.isSystemVariable(variableName) ||
  ...
  async getStats(): Promise<TemplateEngineStats> {
  ...
  private validateHandlebarsSyntax(template: EmailTemplate): {
  ...
  private enhanceContext(context: TemplateContext, templateId: string): any {
    return {
      ...context,
      system: {
        ...context.system,
        tracking_pixel_url: this.trackingService.generatePixelUrl(
          templateId,
          context.contact.id,
          context.campaign?.id
        ),
        unsubscribe_url: this.trackingService.generateUnsubscribeUrl(
          context.contact.id,
          context.campaign?.id
        ),
        view_in_browser_url: this.trackingService.generateViewInBrowserUrl(
          templateId,
          context.contact.id,
          context.campaign?.id
        ),
  ...
  private processHtmlTracking(
    ...
    if (this.config.tracking.enableOpenTracking) {
      const pixelHtml = this.trackingService.generateTrackingPixelHtml(
        templateId,
        context.contact.id,
        context.campaign?.id
      );
    ...
    if (this.config.tracking.enableClickTracking) {
      ...
      processedHtml = processedHtml.replace(
        ...
        (match, prefix, url, suffix) => {
          ...
          const trackingUrl = this.trackingService.generateClickTrackingUrl(
            url,
            templateId,
            context.contact.id,
            context.campaign?.id
    ...
    if (
      ...
      const footerHtml = this.trackingService.generateUnsubscribeFooterHtml(
        context.contact.id,
        context.campaign?.id
      );
  ...
  private async validateContextVariables(
  ...
  private getVariableValue(
    ...
    if (
      context.contact[variableName as keyof typeof context.contact] !==
      undefined
    ) {
      return context.contact[variableName as keyof typeof context.contact];
    }
    if (
      context.campaign?.[variableName as keyof typeof context.campaign] !==
      undefined
    ) {
      return context.campaign[variableName as keyof typeof context.campaign];
    }
    if (
      context.system[variableName as keyof typeof context.system] !== undefined
    ) {
      return context.system[variableName as keyof typeof context.system];
    }
    return undefined;
  }
  ...
  private extractUsedVariables(
    ...
    return template.variables
      .filter(
        (variable) =>
          this.getVariableValue(variable.name, context) !== undefined
      )
  ...
  private isHandlebarsHelper(variableName: string): boolean {
    const helpers = [
      "if",
      "unless",
      "each",
    ...
    const startsWithHelper = [...helpers, ...customHelpers].some(helper => 
      variableName.startsWith(helper + " ") || variableName === helper
    );
  ...
  private updateRenderingStats(renderTime: number, success: boolean): void {
    if (success) {
