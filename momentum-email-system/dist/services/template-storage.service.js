...
exports.SupabaseTemplateStorage = void 0;
...
class SupabaseTemplateStorage {
    ...
    extractBasicVariables(template) {
        const variables = [];
    ...
    async searchTemplates(query, limit = 50) {
        try {
            ...
            const { data: nameResults, error: nameError } = await supabase_1.supabase
                .from("email_templates")
                .select("*")
            ...
            const { data: subjectResults, error: subjectError } = await supabase_1.supabase
                .from("email_templates")
                .select("*")
            ...
            const uniqueResults = allResults.filter((template, index, self) => index === self.findIndex(t => t.id === template.id));
    ...
    async incrementUsageCount(templateId) {
        try {
            const { error } = await supabase_1.supabase.rpc("increment_template_usage", {
                template_id: templateId,
            });
            if (error) {
                console.warn("Failed to increment template usage count:", error.message);
    ...
    async getTemplateStats(_templateId) {
        try {
...
exports.SupabaseTemplateStorage = SupabaseTemplateStorage;
//# sourceMappingURL=template-storage.service.js.map